...
def compute_node_colors(alignement):
    # Number of users and topics
    N1, N2 = alignement.shape
    ...
    g2_colors = [colormap(norm(i))[:-1] for i in range(N2)]

    # Compute user colors by blending topic colors based on alignment weights
    g1_colors = np.zeros((N1, 3))

    for u in range(N1):
        g1_colors[u] = np.sum(g2_colors * alignement[u][:, None], axis=0)
        ...
        for z1, g in enumerate(self.graphs[:-1]):
            ...
            if node_pairs is not None:
                for node1, node2 in node_pairs:
                    print(f"Adding edge between {node1} (layer {z1}) and {node2} (layer {z2})")
                    print(node1 in g.nodes(), node2 in self.graphs[z2].nodes())
                    if node1 in g.nodes() and node2 in self.graphs[z2].nodes():
                        self.edges_between_layers.extend([((node1, z1), (node2, z2))])
                        print(f"Edge added: {node1} (layer {z1}) -> {node2} (layer {z2})")
    def get_node_positions(self,*args, **kwargs):
        """Get the node positions in the layered layout."""
        for z, g in enumerate(self.graphs):
            print(f"Computing layout for layer {z} with {len(g.nodes())} nodes and {len(g.edges())} edges")
            if z == 0:
                pos = self.layout[z](g, *args, **kwargs)
                print(f"Layout for layer {z} computed with {len(pos)} nodes")
